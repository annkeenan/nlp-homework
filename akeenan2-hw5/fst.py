...
class Transition(object):
    ...
    def __str__(self):  # add a way to print out the transitions
        return str(self.q) + ' -' + str(self.a) + '-> ' + str(self.r)


class FST(object):
    def __init__(self):
        self.states = set()
        self.transitions_from = collections.defaultdict(
            lambda: collections.defaultdict(float))
        self.transitions_to = collections.defaultdict(
            lambda: collections.defaultdict(float))
        self.transitions_on = collections.defaultdict(
            lambda: collections.defaultdict(float))
        self.start = None
        self.accept = None
        ...
        for q in self.states:
            ...
            for t, wt in self.transitions_from[q].items():
                ...
                if t.a[0] == EPSILON:
                    ...
                    self.reweight_transition(
                        t, (wt + add) / s[t.a[0]] * (1 - s[EPSILON] / z))
    ...
    def visualize(self):
        ...
        for q in self.states:
            ...
            for r in ts:
                ...
                label = label + "/{}".format(self.transitions_from[q][t])
                dot.append(
                    '{}->{}[label={},fontname=Courier];'.format(index[q], index[r], escape(label)))
        dot.append("}")
        dot = "\n".join(dot)
        proc = subprocess.run(["dot", "-T", "jpg"],
                              input=dot.encode("utf8"), stdout=subprocess.PIPE)
        ...
        for t1, wt1 in m1.transitions_on[a].items():
            if t1.a[1] != EPSILON:
                for t2, wt2 in m2.transitions_on.get(t1.a[1], {}).items():
                    t = Transition(
                        (t1.q, t2.q), (t1.a[0], t2.a[1]), (t1.r, t2.r))
